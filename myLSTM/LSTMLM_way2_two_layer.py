...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.wii = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.whi = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.wif = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.whf = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.wig = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.whg = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.wio = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.who = nn.Linear(emb_size,2*n_hidden,bias=True)

        self.wii_two = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.whi_two = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.wif_two = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.whf_two = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.wig_two = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.whg_two = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.wio_two = nn.Linear(emb_size,2*n_hidden,bias=True)
        self.who_two = nn.Linear(emb_size,2*n_hidden,bias=True)
    ...
    def forward(self, X):
        ...
        for x_t in X:
            ...
            #第二层
            x_t_two = h_t_1
            it_two = self.sigmoid(self.wii_two(x_t_two)+self.whi_two(h_t_1_two))
            ft_two = self.sigmoid(self.wif_two(x_t_two)+self.whf_two(h_t_1_two))
            gt_two = self.tanh(self.wig_two(x_t_two)+self.whg_two(h_t_1_two))
            ot_two = self.sigmoid(self.wio_two(x_t_two)+self.who_two(h_t_1_two))
        ...
        outputs = ht_two # [batch_size, num_directions(=1) * n_hidden]
